...
class QKPOptimizer:
    ...
    def apply_shallow_copula_mixer(self, qc, beta, p1, p2, theta, return_circ=False):
    ...
    def QKP(self, betas, gammas, k, theta=None, bit_mapping='regular',
        ...
        # The good one corresponding to the paper
        elif self.mixer == 'copula_2':
            print("Using copula_2")
    ...
    def initial_state_preparation(self, qc, p):
        """Prepare the initial state of the quantum circuit."""
        for i in range(self.n):
            angle = 2 * np.arcsin(np.sqrt(p[i]))
            qc.ry(angle, i)
        qc.barrier()
    ...
    def generate_circuits(self, k_range, theta_range, beta_values, gamma_values,
                          warm_start_only=False, all_params=None):
                ...
                if all_params is None:
                    for beta in beta_values:
                        for gamma in gamma_values:
                            p_dist = self.logistic_bias(k)
                            qc = QuantumCircuit(self.n)
                                
                            # Initial state preparation
                            for i in range(self.n):
                                angle = 2 * np.arcsin(np.sqrt(p_dist[i]))
                                qc.ry(angle, i)
                            qc.barrier()
                            ...
                            if self.mixer == 'X':
                                ...
                                self.apply_hourglass_mixer(qc, beta, p_dist)

                            elif self.mixer == 'copula_2':
                                # p2 = self.logistic_bias(k)
                                self.ring_copula_mixer(qc, p_dist, theta, beta, trotter_steps=1)
                                qc.barrier()
                ...
                if all_params is not None:
                    for beta, gamma in all_params:
                        ...
                        for layer in range(self.p):
                            print(f"Layer {layer+1} -- (k, θ): {int(k), theta}")

                            # Cost unitary
                            self.apply_cost_unitary(qc, gamma[layer])
                            qc.barrier()
                            ...
                            if self.mixer == 'X':
                                ...
                                # p2 = self.logistic_bias(k)
                                self.ring_copula_mixer(qc, p_dist, theta, beta[layer], trotter_steps=1)
                                qc.barrier()
